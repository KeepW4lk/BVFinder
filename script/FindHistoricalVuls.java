...
public class RQ1 extends GhidraScript {
    ...
    protected boolean analyze() {
     ...
     memcpy = GlobalState.flatAPI.getFunctionAt(GlobalState.flatAPI.toAddr(0x40057f0)); // add sink function
     BasebandMemcpy.addstaticSymbols(memcpy.getName());
     addresses.clear();
     addresses.add(GlobalState.flatAPI.toAddr(0x416050d4));  // add taint source
     addresses.add(GlobalState.flatAPI.toAddr(0x42b3d604));  // add taint source
     BVFinder.tagTaintSource(addresses);
     // set start point
     analyzeFromAddress(GlobalState.flatAPI.toAddr(0x407087ac));
     analyzeFromAddress(GlobalState.flatAPI.toAddr(0x408a8938));
     return true;
    }
